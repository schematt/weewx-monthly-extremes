...
1. **`generate_extremes.py`** — Server-side script
...
1. Rename `records-mensili.html` to `records-extremos.html`
2. Edit `nav-menu-custom.inc`:
   ```html
   <span itemprop="name">Registros Mensuales</span>
   ```
3. Update URLs in the script if needed
...
Found a bug? Have a suggestion? Please open an [issue](https://github.com/YOUR_USERNAME/REPO_NAME/issues) or submit a [pull request](https://github.com/YOUR_USERNAME/REPO_NAME/pulls).

### Development

To modify the report:

1. Test `generate_extremes.py` locally:
   ```bash
   python3 generate_extremes.py
   ```

2. Edit `records-mensili.html` in a browser for styling

3. Test in dark mode: Press `Shift + Cmd/Ctrl + L` in browser dev tools
...
**Created for:** [pnmeteo.it](https://www.pnmeteo.it/ecowitt_new/)  
**WeeWX Skin:** [New Belchertown](https://github.com/uajqq/weewx-belchertown-new)
